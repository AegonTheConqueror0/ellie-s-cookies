...
export default function Calculator() {
  ...
  const updateOverhead = (id: string, updates: Partial<Overhead>) => {
    setOverheads(
      overheads.map((o) => (o.id === id ? { ...o, ...updates } : o))
    );
  };
  ...
  const calculations = useMemo(() => {
    ...
    const profitPerCookie = sellingPrice - costPerCookie;
    const totalProfitPerBatch = (sellingPrice * batchSize) - totalBatchCost;
  ...
  }, [ingredients, overheads, batchSize, sellingPrice]);
    ...
    <div className="max-w-5xl mx-auto">
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                      <div className="md:col-span-6">
                        <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Overhead Name</label>
                        <input
                          type="text"
                      ...
                      <div className="md:col-span-4">
                        <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cost per Batch</label>
                        <input
                          type="number"
                          placeholder="₱"
                          value={o.cost || ''}
                          onChange={(e) => updateOverhead(o.id, { cost: parseFloat(e.target.value) || 0 })}
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all"
                        />
                      </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-md border border-slate-200 sticky top-8">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Cost / Cookie</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-black text-slate-900">₱{calculations.costPerCookie.toFixed(2)}</span>
                  </div>
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Selling Price</span>
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      className="w-full text-2xl font-black text-amber-600 bg-transparent border-none focus:ring-0 p-0"
                      placeholder="0.00"
                    />
                  </div>
                </div>
              ...
              <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-xs font-bold text-emerald-800 uppercase tracking-widest">Profit / Cookie</span>
                  <span className={`text-xl font-black ${calculations.profitPerCookie >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    ₱{calculations.profitPerCookie.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-emerald-700 uppercase">Total Batch Profit</span>
                  <span className={`text-sm font-bold ${calculations.totalProfitPerBatch >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    ₱{calculations.totalProfitPerBatch.toFixed(2)}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Ingredients Cost</span>
                  <span className="text-lg font-bold text-slate-700">₱{calculations.totalIngredientsCost.toFixed(2)}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Overhead Cost</span>
                  <span className="text-lg font-bold text-slate-700">₱{calculations.totalOverheadCost.toFixed(2)}</span>
                </div>
              </div>
      ...
      <div className="fixed bottom-8 right-8 z-40 flex flex-col gap-3 items-end">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={addOverhead}
          className="flex items-center gap-2 px-6 py-4 bg-slate-700 hover:bg-slate-800 text-white rounded-full transition-all font-bold shadow-2xl shadow-slate-200 group"
        >
          <Plus className="w-6 h-6 transition-transform group-hover:rotate-90" />
          <span className="hidden md:inline">Add Overhead</span>
        </motion.button>
        ...
        <motion.button
          ...
          className="flex items-center gap-2 px-6 py-4 bg-amber-600 hover:bg-amber-700 text-white rounded-full transition-all font-bold shadow-2xl shadow-amber-200 group"
        >
          <Plus className="w-6 h-6 transition-transform group-hover:rotate-90" />
